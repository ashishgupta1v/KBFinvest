...
const insurers = computed(() => props.data.insurers || {});
const currentService = computed(() => services.value.find((s) => s.id === activeTab.value) || services.value[0]);
</script>

<template>
  <AppLayout>
    <Head title="Insurance — Health, Life & Motor Protection in Ludhiana" />
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="max-w-3xl space-y-3">
          ...
          <h1 class="text-3xl sm:text-5xl font-bold font-display">
            Protect the plan <span class="italic text-gold-gradient font-serif">before</span> you build it
          </h1>
          <p class="text-base text-kb-body leading-relaxed">
            Health, life and motor cover chosen on what it actually pays out — not on the premium alone. We read the exclusions with you before you sign, and we stay on at claim time.
          </p>
        </div>
    ...
    <section v-if="currentService" class="py-12 border-b border-kb-line">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
          ...
          <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center w-full sm:w-auto gap-2.5">
              ...
              >
                ...
                <span>Calculate Cover Need</span>
              </Link>
              <Link
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div class="card-luxury p-6 space-y-4">
            <h3 class="text-base font-bold text-kb-text flex items-center gap-2">
              <SvgIcon name="i-shield" className="w-4 h-4 text-kb-accent" />
              <span>Clauses that actually matter</span>
            </h3>
            <ul class="space-y-2.5 text-xs text-kb-body">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="card-frame p-6 space-y-4">
            <h3 class="text-sm font-bold uppercase tracking-wider text-kb-muted">Policy Facts</h3>
            <div class="space-y-3">
              <div
          ...
          <div class="card-frame p-6 space-y-4">
            <h3 class="text-sm font-bold uppercase tracking-wider text-kb-muted">Documents Required</h3>
          ...
          <div class="card-frame p-6 space-y-4">
            <h3 class="text-sm font-bold uppercase tracking-wider text-kb-muted">Steps to Issuance</h3>
    ...
    <!-- INSURER NETWORK DIRECTORY -->
    <section class="py-16 bg-kb-surface/30">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        <div class="max-w-3xl space-y-2">
          <h2 class="text-2xl sm:text-3xl font-bold font-display">
            Insurance Partners & <span class="italic text-gold-gradient font-serif">Networks</span>
          </h2>
          <p class="text-sm text-kb-muted">
            We introduce and compare across regulated general, health, and life insurers. Insurer names appear for identification; benefits and claims depend solely on policy wording.
          </p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div v-for="(cat, key) in insurers" :key="key" class="card-frame p-6 space-y-4">
            ...
            <ul class="space-y-2 text-xs text-kb-body">
              <li v-for="(item, idx) in cat.list" :key="idx" class="flex items-center gap-2 py-1 border-b border-kb-line/30 last:border-0">
                <span class="w-1.5 h-1.5 rounded-full bg-kb-line"></span>
                <span>{{ item }}</span>
              </li>
            </ul>
          </div>
